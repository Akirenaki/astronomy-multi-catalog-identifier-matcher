...
- **A personal Gemini API key (planned, not yet implemented).** The intent is that from `/account/settings`, you'll be able to add your own Gemini API key and, optionally, a preferred model, as a *fallback only* — every generation would still try the app's own shared key first, with your key used only if that comes back rate-limited/quota-exhausted. **This does not exist in the code yet**: there's no `/account/settings` route, no key column on the user model, and no per-user parameter anywhere in the summary-generation path. The `USER_SECRET_ENCRYPTION_KEY` config variable below is provisioned for this future feature but currently has nothing to encrypt.
...
- **No personal summary snapshot.** You always see the current shared summary as-is. (The personal-API-key fallback mentioned above isn't built yet either, for anyone.)
...
2. **Create a `.env` file** in `app/` (a repo-root `.env` also works for `GEMINI_API_KEY`/`SESSION_SECRET_KEY` specifically, but `app/.env` is the one that reliably works for everything below; see the caveat under `DATABASE_URL`/`USER_SECRET_ENCRYPTION_KEY`):
   ...
   | `USER_SECRET_ENCRYPTION_KEY` | Not yet used | Provisioned for the planned personal-Gemini-key feature (see III.A) — encrypting personal Gemini API keys at rest — but nothing in the code reads or writes a personal key yet, so this variable currently has no effect either way. As with `DATABASE_URL` above, a `.env`-only value would be honoured once the feature exists, since `load_environment()` already runs before any config variable is read. |
...
9. **On a Generate/Regenerate request**, call Gemini using the app's own shared key and persist the result as the one shared `ai_summary` for that object, kept strictly separate from the scientific data so the AI layer can never corrupt or override what the SQL layer already established. (The personal-key retry-on-rate-limit described in III.A is planned but not yet implemented — today there is only the one shared key.)
